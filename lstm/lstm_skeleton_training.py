...
class SkeletonDataset(Dataset):
    def __init__(self, data_file, seq_length=181):
    ...
    def __getitem__(self, idx):
        ...
        target = 1  # 'fall_down' 동작을 나타내는 레이블
...
# 데이터셋 및 데이터로더 생성
train_dataset = SkeletonDataset('./fall_down_result/skeleton_fall_down_train.txt', seq_length=seq_length)
...
for epoch in range(num_epochs):
    ...
    for inputs, labels in train_loader:
        ...
        loss = criterion(outputs, labels.unsqueeze(1))  # 레이블을 (batch_size, 1)로 변환하여 손실 계산
