...
if __name__ == "__main__":
    ...
    base_output_dir = './data/detection_C'
    ...
    while cap.isOpened():
        ...
        person_results = person_model.predict(frame, save=False, classes=[0], conf=0.5)

        # 각 모델의 결과를 필터링
        filtered_wheelchair_results = filter_boxes(wheelchair_results)
        filtered_knife_results = filter_boxes(knife_results)
        filtered_person_results = filter_boxes(person_results)

        # 결과를 하나의 리스트로 합치기
        all_filtered_results = filtered_wheelchair_results + filtered_knife_results + filtered_person_results
        ...
        # 100프레임마다 person 모델의 감지된 사람 수를 출력
        if frame_count % 100 == 0:
            person_count = sum(len(result) for result in filtered_person_results)
            logging.info(f"Frame {frame_count}: Detected {person_count} person(s)")
